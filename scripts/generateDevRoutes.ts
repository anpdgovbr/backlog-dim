...
const getRoutes = (dir: string, baseUrl = ""): string[] => {
  console.log(`📂 Lendo diretório: ${dir}`) // Log de cada diretório acessado
    ...
    for (const file of files) {
      ...
      if (file.isDirectory()) {
        ...
        if (!["node_modules", ".git", "public"].includes(file.name)) {
          console.log(`🔍 Entrando em: ${fullPath}`)
        ...
        console.log(`✅ Adicionando rota: ${routePath}`)
      }
    }
  } catch (error) {
...
console.log("🔎 Buscando páginas e APIs...")
...
try {
  ...
  console.log("✅ JSON de rotas gerado com sucesso:", outputPath)
} catch (error) {
