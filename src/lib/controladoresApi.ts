...
export async function parseControladoresJson<T>(response: Response): Promise<T | null> {
  ...
  console.warn("🔍 [parseControladoresJson] Primeiros 500 chars:", raw.slice(0, 500))
  ...
  try {
    ...
    if (parsed && typeof parsed === "object" && "data" in parsed) {
      console.warn("🔍 [parseControladoresJson] Array data tem", (parsed as {data: unknown[]}).data?.length ?? 0, "itens")
    }
    return parsed
  } catch (error) {
