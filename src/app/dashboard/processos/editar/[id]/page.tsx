...
function EditarProcessoContent() {
  ...
  const handleSubmit = methods.handleSubmit(async (dataFromForm) => {
    ...
    if (res.ok) {
      ...
      mutate(`/api/processos/${id}`) // Atualiza o cache
    } else {
  ...
  if (isLoading || !processo) return <p>Carregando dados do processo...</p>
...
const EditarProcessoPage = withPermissao(
  EditarProcessoContent,
  "EditarGeral",
  "Processo",
  {
    redirecionar: false,
